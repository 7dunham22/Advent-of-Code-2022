...
maxWidth += 1
maxDepth += 2
minWidth = min(minWidth, 500 - maxDepth)
maxWidth = max(maxWidth, 500 + maxDepth)

cave = [['.' for x in range(maxWidth+1 - minWidth)] for _ in range(maxDepth+1)]
n = len(cave)
m = len(cave[0])
cave[n-1] = ['#' for _ in range(m)]
...
def fallSand(r, c):
  ...
  if c < m-1 and cave[r+1][c+1] != 'o' and cave[r+1][c+1] != '#':
    return fallSand(r+1,c+1)
  cave[r][c] = 'o'
  if r == sandStart[0] and c == sandStart[1]:
    return True
  return False
...
def getMaxSand():
  ...
  while True:
    res += 1
    isBlocked = fallSand(sandStart[0], sandStart[1])
    if isBlocked:
      return res
